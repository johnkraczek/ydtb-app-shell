 ...
 * import { PortalProvider, PortalSlot, usePortal } from '@ydtb/core-client'
 *
 ...
 * import { Portal } from '@ydtb/core-client'
 *
...
interface PortalRegistryValue {
  ...
  /** Get the entire metadata bag for a slot (snapshot copy). */
  getSlotMetadataAll: (name: string) => Record<string, unknown>
...
export function PortalProvider({ children }: PortalProviderProps) {
  ...
  const value = useMemo<PortalRegistryValue>(
    () => ({
      ...
      getSlotMetadataAll: (name) => ({ ...(slotsRef.current.get(name)?.metadata ?? {}) }),
...
export function usePortalMeta(name: string): {
  ...
  useEffect(() => {
    return registry.subscribe(name, () => {
      setLocal(registry.getSlotMetadataAll(name))
    })
  }, [name, registry])
